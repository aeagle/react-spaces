	...
	return (
		<Space.Fill className="layers-demo">
			<Space.Top size={40} centerContent={Space.CenterType.HorizontalVertical}>
				<label>
					<input type="checkbox" checked={showLayer1} onChange={(e) => setShowLayer1(e.target.checked)} /> Show layer 1
				</label>
				<label>
					<input type="checkbox" checked={showLayer2} onChange={(e) => setShowLayer2(e.target.checked)} /> Show layer 2
				</label>
			</Space.Top>
			<Space.Fill>
				{showLayer1 && (
					<Space.Layer zIndex={0}>
						...
						<Space.Fill as="main" id="layer_1_main">
							{Description(
								"Fill space not affected by floated left space in different layer but is affected by left space in same layer (zIndex = 0)",
							)}
						</Space.Fill>
					</Space.Layer>
				)}
				...
				{showLayer2 && (
					<Space.Layer zIndex={1}>
						...
						<Space.Left id="layer_2_left1" order={2} className="layer2" size="15%" as="section">
							{Description("Left floated (zIndex = 1)")}
						</Space.Left>
						<Space.Fill as="main" className="layer2" id="layer_2_main">
							{Description(
								"Fill space not affected by floated left space in different layer but is affected by left space in same layer (zIndex = 0)",
							)}
						</Space.Fill>
