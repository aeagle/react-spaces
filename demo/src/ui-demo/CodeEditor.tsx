...
const Header = () => {
	return (
		<Space.Top
			...
			size={30}
			style={{ backgroundColor: "#333", color: "#c5c5c5" }}>
			<MenuBar />
...
const MenuBar = () => {
	return <Space.Left className="menu-bar" size="30%"></Space.Left>;
};
...
const Editor: React.FC<{ toggleVisibility: () => void; toggleSize: () => void }> = (props) => {
	...
	return (
		<Space.Fill>
			<Space.Fill>
				<Space.Top className="editor-tabs" size={40}></Space.Top>
...
const IconPane = () => {
	return <Space.Left order={1} className="side-bar-icons" size={50} style={{ backgroundColor: "#333" }}></Space.Left>;
};
...
const SideBar: React.FC<{ wide: boolean; visible: boolean }> = (props) => {
	return (
		<Space.LeftResizable
			order={2}
			className="side-bar"
			size={props.visible ? (props.wide ? 500 : 300) : 0}
			handleSize={20}
			overlayHandle={true}
			style={{ backgroundColor: "#252526" }}>
			<SideBarPane order={1} title="Open editors" height={200}></SideBarPane>
			<SideBarPane order={2} title="Demo" height={300}></SideBarPane>
			<SideBarPane fill={true} title="Outline"></SideBarPane>
		</Space.LeftResizable>
	);
};
	...
	return (
		<>
			<Space.Top className="title" size={28} style={{ backgroundColor: "#383838", color: "#c5c5c5" }}>
				<Space.CenteredVertically>{props.title}</Space.CenteredVertically>
			</Space.Top>
